...
void DrawPrism(const int n)
{
    ...
    const int vn = 2 * n + 1; // 2 сечения + 1 средняя точка

    float vertices[3 * vn]; // Это вершины призмы

    for (int i = 0; i < vn; i++) {
        float phi = 2 * pi * (float) i / (float) (vn - 1); // Среднее не равно
        float c = cos(phi);
        float s = sin(phi);

        // Вершины в верхнем сечении
        vertices[3 * i] = 0.5 * c; // Можно изменить радиус верхнего сечения
        vertices[3 * i + 1] = 0.5 * s; // Можно изменить радиус верхнего сечения
        vertices[3 * i + 2] = 1.0;

        // Вершины в нижнем сечении
        vertices[3 * (i + vn)] = c; // Можно изменить радиус нижнего сечения
        vertices[3 * (i + vn) + 1] = s; // Можно изменить радиус нижнего сечения
        vertices[3 * (i + vn) + 2] = -1.0;
    }
    ...
    // Верхнее сечение
    glDrawArrays(GL_POLYGON, 0, vn);

    // Нижнее сечение
    glDrawArrays(GL_POLYGON, vn, vn);
